...
const testimonials = [
  ...
  {
    quote: "The polish, speed, and architecture quality are unlike any software partner we used before.",
    name: "Daniel Kim",
    role: "CTO",
    company: "StrataOne",
  },
...
const HeroNetwork = () => {
  return (
    <svg viewBox="0 0 540 420" className="hero-network" aria-hidden="true">
      ...
      <path d="M90 80 L220 120 L330 65 L470 110" stroke="url(#sovertickGradient)" strokeWidth="2" fill="none" className="network-line" />
      <path d="M90 210 L220 120 L280 240 L410 260" stroke="url(#sovertickGradient)" strokeWidth="2" fill="none" className="network-line delay" />
      <path d="M120 320 L280 240 L360 340 L470 290" stroke="url(#sovertickGradient)" strokeWidth="2" fill="none" className="network-line" />
      {[{ x: 90, y: 80 }, { x: 220, y: 120 }, { x: 330, y: 65 }, { x: 470, y: 110 }, { x: 280, y: 240 }, { x: 120, y: 320 }, { x: 470, y: 290 }].map(
        (node, i) => (
          <circle key={i} cx={node.x} cy={node.y} r="6" fill="url(#sovertickGradient)" className="network-node" />
        ),
      )}
      <rect x="185" y="148" width="180" height="110" rx="16" fill="rgba(255,255,255,0.05)" stroke="rgba(176,110,255,0.35)" />
...
const Index = () => {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setActiveTestimonial((prev) => (prev + 1) % testimonials.length), 4200);
    return () => window.clearInterval(timer);
  }, []);
  ...
  const techStack = useMemo(
    () => ["React", "Node.js", "Python", "Flutter", "AWS", "MongoDB", "TypeScript", "Docker", "Next.js", "Figma"],
    [],
  );

  return (
    <div className="sovertick-root">
      <motion.div className={`cursor-dot-lite ${cursorHover ? "cursor-active" : ""}`} animate={{ x: cursorPos.x - 6, y: cursorPos.y - 6 }} transition={{ type: "spring", stiffness: 230, damping: 24 }} />

      <AnimatePresence>{showLoader && <IntroLoader />}</AnimatePresence>

      <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: showLoader ? 0 : 1, y: showLoader ? 28 : 0 }} transition={{ duration: 0.6 }}>
        <header className={`sovertick-nav ${navbarSolid ? "nav-solid" : ""}`}>
          <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
            ...
            <nav className="hidden items-center gap-8 lg:flex">
              {navLinks.map((link) => (
                <a key={link.id} href={`#${link.id}`} className={`nav-link-dot ${activeSection === link.id ? "active" : ""}`} data-cursor-hover="true">
                  {link.label}
                </a>
              ))}
            </nav>
            <MagneticButton href="#contact" className="cta-main hidden items-center gap-2 rounded-full px-5 py-2.5 lg:inline-flex">
              Let's Build <ArrowRight className="h-4 w-4" />
            </MagneticButton>
            <button type="button" onClick={() => setMobileMenu((v) => !v)} className="lg:hidden" data-cursor-hover="true">
              {mobileMenu ? <X className="h-6 w-6 text-[#F5F5F7]" /> : <Menu className="h-6 w-6 text-[#F5F5F7]" />}
            </button>
          </div>
          <AnimatePresence>
            {mobileMenu && (
              <motion.div initial={{ opacity: 0, y: -18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} className="mobile-menu">
                {navLinks.map((link, index) => (
                  <motion.a key={link.id} href={`#${link.id}`} className="mobile-link" onClick={() => setMobileMenu(false)} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }}>
                    {link.label}
                  </motion.a>
                ))}
              </motion.div>
        ...
        <main>
          <Section id="home" className="section-hero px-6 pb-20 pt-28 lg:px-10 lg:pt-32">
            <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-2">
              <div>
                <motion.p initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2, type: "spring" }} className="hero-badge">
                  ✦ Global Software Excellence
                </motion.p>
                <div className="mt-7 space-y-2">
                  <motion.h1 className="hero-title" initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }}>
                    We Don't Build Software.
                  </motion.h1>
                  <motion.h1 className="hero-title gradient-title shimmer-text" initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    We Build What's Next.
                  </motion.h1>
                </div>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="mt-6 max-w-xl text-lg italic text-[#6E7191]">
                ...
                <motion.div initial={{ opacity: 0, y: 16, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.8 }} className="mt-8 flex flex-wrap gap-4">
                  <MagneticButton href="#work" className="cta-main pulse-ring inline-flex items-center gap-2 rounded-full px-6 py-3">
                    See Our Work <ArrowRight className="h-4 w-4" />
                  </MagneticButton>
                  <MagneticButton href="#about" className="cta-ghost inline-flex items-center gap-2 rounded-full px-6 py-3">
                    Our Story
                  </MagneticButton>
                </motion.div>
              </div>

              <motion.div className="hero-visual-wrap" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                <div className="hero-orb-left" />
                <HeroNetwork />
              </motion.div>
            </div>
            <motion.a href="#services" className="scroll-indicator" animate={{ y: [0, 6, 0] }} transition={{ duration: 1.4, repeat: Infinity }} data-cursor-hover="true">
          ...
          <section className="section-marquee py-4">
            <div className="marquee-edge">
              <div className="marquee-track">
                <span>Web Development ✦ Mobile Apps ✦ AI Solutions ✦ Cloud Architecture ✦ UI/UX Design ✦ Cybersecurity ✦ Data Engineering ✦</span>
                <span>Web Development ✦ Mobile Apps ✦ AI Solutions ✦ Cloud Architecture ✦ UI/UX Design ✦ Cybersecurity ✦ Data Engineering ✦</span>
              </div>
            </div>
          </section>
            ...
            <div className="mx-auto max-w-7xl">
              ...
              <div className="mt-12 grid gap-6 md:grid-cols-3">
                {serviceCards.map((card, index) => (
                  <motion.article key={card.title} initial={{ opacity: 0, scaleY: 0.06, transformOrigin: "top" }} whileInView={{ opacity: 1, scaleY: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: index * 0.15, ease: [0.34, 1.56, 0.64, 1] }} className={`service-card ${card.className}`}>
                    ...
                    <span className="mt-5 inline-flex rounded-full border border-[#B06EFF33] px-3 py-1 text-xs text-[#B06EFF]">{card.tag}</span>
                    <a href="#contact" className="mt-5 inline-flex items-center gap-1 text-sm text-[#F5F5F7]" data-cursor-hover="true">
          ...
          <section className="section-marquee py-4">
            <div className="marquee-edge">
              <div className="marquee-track reverse">
                <span>React ✦ Node.js ✦ Python ✦ Flutter ✦ AWS ✦ TypeScript ✦ Docker ✦ MongoDB ✦ Next.js ✦ Figma ✦</span>
                <span>React ✦ Node.js ✦ Python ✦ Flutter ✦ AWS ✦ TypeScript ✦ Docker ✦ MongoDB ✦ Next.js ✦ Figma ✦</span>
              </div>
            </div>
          </section>

          <section ref={statsRef as React.Ref<HTMLElement>} className="section-stats px-6 py-14 lg:px-10">
            <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-4">
              {[
                ...
                <motion.div key={item.label} initial={{ opacity: 0, y: 18 }} animate={statsInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: index * 0.12 }} className="stat-tile">
                  <p className="stat-number">
                    <ScrambleCounter value={item.n} start={statsInView} suffix={item.suffix ?? "+"} />
                  </p>
          ...
          <Section className="section-process px-6 py-20 lg:px-10">
            <div className="mx-auto max-w-7xl" ref={processRef as React.Ref<HTMLDivElement>}>
              ...
              <div className="relative mt-12 grid gap-6 md:grid-cols-4">
                <motion.div className="process-line" initial={{ scaleX: 0 }} animate={processVisible ? { scaleX: 1 } : {}} transition={{ duration: 1.1 }} />
                {processSteps.map((step, index) => (
                  <motion.article key={step.step} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="process-card">
          ...
          <Section id="work" className="section-portfolio px-6 py-20 lg:px-10">
            <div className="mx-auto max-w-7xl">
              ...
              <div ref={portfolioRef} onScroll={onPortfolioScroll} className="portfolio-scroll mt-10">
                {portfolio.map((item) => (
                  <article key={item.name} className="portfolio-card">
                    ...
                    <div className="portfolio-overlay">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {item.tags.map((tag) => (
                          <span key={tag} className="rounded-full border border-[#ffffff22] px-3 py-1 text-xs text-[#F5F5F7]">
                            {tag}
                          </span>
                        ))}
                      </div>
                      <a href="#contact" className="mt-5 inline-flex items-center gap-2 text-[#F5F5F7]" data-cursor-hover="true">
              ...
              <div className="mt-5 flex justify-center gap-2">
                {portfolio.map((_, i) => (
                  <button key={i} type="button" className={`dot ${portfolioDot === i ? "dot-active" : ""}`} onClick={() => jumpPortfolio(i)} data-cursor-hover="true" aria-label={`Go to project ${i + 1}`} />
                ))}
              </div>
            </div>
          </Section>
          ...
          <Section className="section-tech px-6 py-20 lg:px-10">
            <div className="mx-auto max-w-7xl">
              <h2 className="section-title">
                <HeadingReveal text="Technologies We" /> <span className="gradient-title">Master</span>
              </h2>
            ...
            <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
              <div>
                ...
                <p className="mt-6 max-w-xl text-[#6E7191]">
                  We combine product depth, visual craft, and operational discipline to ship software that performs globally and feels unmistakably human.
                </p>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {features.map((feature, index) => (
                  <motion.div key={feature} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.07 }} className="feature-tile">
            ...
            <div className="mx-auto max-w-7xl">
              ...
              <div className="testimonial-stack mt-12">
                {testimonials.map((item, i) => {
                  const distance = (i - activeTestimonial + testimonials.length) % testimonials.length;
                  const position = distance === 0 ? 0 : distance === 1 ? 1 : -1;
                  return (
                    <motion.article key={item.name} className="testimonial-card" animate={{ x: position * 60, scale: position === 0 ? 1 : 0.92, opacity: position === 0 ? 1 : 0.5, zIndex: position === 0 ? 3 : 1 }} transition={{ duration: 0.5 }}>
              ...
              <div className="mt-6 flex justify-center gap-2">
                {testimonials.map((_, index) => (
                  <button key={index} onClick={() => setActiveTestimonial(index)} className={`dot ${index === activeTestimonial ? "dot-active" : ""}`} type="button" data-cursor-hover="true" />
                ))}
              </div>
            </div>
          </Section>
            ...
            <div className="mx-auto max-w-7xl">
              ...
              <div className="mt-10 grid gap-6 md:grid-cols-3">
                {visionCards.map((card, index) => (
                  <motion.article key={card.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.12 }} className="vision-card">
          ...
          <Section id="careers" className="section-cta px-6 py-20 lg:px-10">
            <div className="mx-auto max-w-6xl text-center">
              ...
              <p className="section-poetic text-center">Let's turn bold ideas into products the world notices.</p>
              <div className="mt-8">
                <MagneticButton href="#contact" className="cta-invert inline-flex items-center gap-2 rounded-full px-8 py-4">
          ...
          <Section id="contact" className="section-contact px-6 py-20 lg:px-10">
            <div className="mx-auto max-w-4xl rounded-3xl border border-[#B06EFF44] bg-[#ffffff06] p-8 backdrop-blur-xl">
              ...
              <p className="section-poetic text-center">Tell us what you're building and we'll make it real.</p>
              <form className="mt-8 grid gap-4 md:grid-cols-2" onSubmit={onSubmit}>
                ...
                <textarea className="input-premium min-h-32 md:col-span-2" placeholder="Message" required />
                <button type="submit" className="cta-main md:col-span-2 inline-flex items-center justify-center gap-2 rounded-full py-3" data-cursor-hover="true">
                  {sending ? "Sending..." : "Submit"}
                  <ArrowRight className="h-4 w-4" />
                </button>
                <AnimatePresence>
                  {submitted && (
                    <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="md:col-span-2 rounded-xl border border-[#FF6B6B55] bg-[#FF6B6B22] px-4 py-3 text-center text-sm text-[#F5F5F7]">
        ...
        <footer className="section-footer px-6 pb-10 pt-14 lg:px-10">
          ...
          <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-5">
            <div className="md:col-span-2">
              ...
              <p className="mt-3 max-w-xs text-[#6E7191]">Engineering tomorrow. Delivering today.</p>
            </div>
            <div>
          ...
          <p className="mx-auto mt-10 max-w-7xl border-t border-[#ffffff12] pt-6 text-sm text-[#6E7191]">© 2025 Sovertick Technologies. All Rights Reserved. | Privacy | Terms</p>
        </footer>
      </motion.div>
    </div>
  );
